...
const PAGE = 'https://cloudsforge.online/worlds/aetherholm/'
...
describe('the in-place network view', () => {
  ...
  it('starts on the network the hostname names, and says so', () => {
    at(PAGE, () => {
      ...
      assert.equal(apiBase(), 'https://cloudsforge.online/worlds/aetherholm')
    })
  })
  ...
  it('re-points this page at the testnet estate WITHOUT navigating anywhere', () => {
    at(PAGE, () => {
      ...
      // ── THE TESTNET APEX PLUS THE SAME MOUNT, WHICH IS THE POINT OF THE NESTING ────────────
      //
      // This was `https://aetherholm-testnet.cloudsforge.online` — a per-title testnet hostname.
      // Since the title became a folder there is no such name: both estates serve this game from
      // the SAME path, and switching network changes only which apex sits in front of it. Which
      // estate (viewed.ts) and where under it (routes.ts BASE) are separate questions, and
      // conflating them is what broke agora's switcher in wave 3c.
      assert.equal(apiBase(), 'https://testnet.cloudsforge.online/worlds/aetherholm')
    })
  })
