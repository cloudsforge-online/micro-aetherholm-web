...
const PAGE = 'https://aetherholm.cloudsforge.online/'
...
describe('the in-place network view', () => {
  ...
  it('starts on the network the hostname names, and says so', () => {
    at(PAGE, () => {
      ...
      assert.equal(apiBase(), 'https://aetherholm.cloudsforge.online')
    })
  })
  ...
  it('re-points this page at the testnet estate WITHOUT navigating anywhere', () => {
    at(PAGE, () => {
      ...
      assert.equal(apiBase(), 'https://aetherholm-testnet.cloudsforge.online')
    })
  })
