...
/**
 * nginx.conf with its comments removed. The file's own header QUOTES the directive it forbids in
 * order to explain why the routes are enumerated by hand; a grep over the raw text matches the
 * warning and fails a correct file. The rule is about DIRECTIVES; strip the prose first.
 */
...
describe('the route declaration', () => {
  ...
  it('marks the play screens protected and the public history public', () => {
    assert.equal(routeFor('/')?.protected, true)
    assert.equal(routeFor('/cities')?.protected, true)
    ...
    // (aetherholm/src/server.ts); battles because a sealed report opens without a session
    // (server.ts) and a live one renders its own refusal honestly.
...
describe('the router renders every declared route', () => {
  ...
  it('does NOT gate the public routes — the chronicle must open for a stranger', () => {
    for (const path of ['/chronicle', '/battles']) {
      const index = appSource.indexOf(`path="${path}"`)
      assert.ok(index > 0)
      // Only THIS route's element: read to the close of its own <Route …/>, not a fixed window
      // that would swallow the next route's gate and fail a correct file.
      const close = appSource.indexOf('/>', index)
      assert.ok(close > index, `the <Route path="${path}"> element never closes`)
      const element = appSource.slice(index, close)
      assert.ok(!element.includes('ProtectedRoute'), `${path} is public but the router gates it`)
    }
  })
})
...
describe('nginx serves every route the router owns', () => {
  ...
  it('REPEATS the security headers in every location that sets a header of its own', () => {
    // nginx's `add_header` inheritance is all-or-nothing per level: a location containing even
    // one `add_header` discards every one from the enclosing block.
    const blocks = directives.match(/location[^{]*\{[^}]*\}/gs) ?? []
    assert.ok(blocks.length >= 5, `found only ${blocks.length} location blocks`)
    const withHeaders = blocks.filter((b) => b.includes('add_header'))
    assert.ok(withHeaders.length >= 4, 'expected several locations to set headers')
