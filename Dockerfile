...
# shipped images with no favicon in them while their own brand tests passed against the source
# tree — the artefact and the repository disagreed. Vite copies public/ into dist/ verbatim at
# build time, and CI probes the RUNNING container for the files.
COPY public ./public
COPY src ./src
