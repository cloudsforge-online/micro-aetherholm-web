 ...
 * `errorReply()` in every service; aetherholm's own is `aetherholm/src/server.ts`. Both
...
export interface RequestOptions {
   ...
   * launch REQUIRES an `Idempotency-Key` and is a 400 without it (`aetherholm/src/server.ts`, `idempotencyKeyOf`).
   * A launch that is retried without the header charges the treasury — and puts a
...
/**
 * `micro-aetherholm` — this game's own service. Always absolute; see `apiBase()`'s note.
 *
 * Nothing calls this directly. Every route goes through a named function in `./aetherholm.ts`,
 * each of which cites the `aetherholm/src/server.ts` line it was verified against. That
 * indirection is not ceremony: this estate has shipped clients written against routes that did
 * not exist, and they survived review because the call site read like an ordinary fetch.
 */
export const aetherholm = <T,>(path: string, opts?: RequestOptions): Promise<T> =>
